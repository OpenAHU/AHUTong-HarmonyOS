...
- 当前模块：`entry` HAP，以及 `core_common`、`core_designsystem`、`data_schedule`、`feature_schedule` HAR
- 当前实现：已建立 `Core → Data → Feature → Entry` 可编译依赖链和 ArkUI 基础设计系统，业务功能尚未迁移
...
| M2 | Design System、主题、通用页面骨架与导航 | 进行中 |
