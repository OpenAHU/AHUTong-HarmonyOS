...
| 天气 | `feature:weather` | `data/weather` + `feature/weather` | 进行中 | 已迁移完整模型、城市/IP/区划码查询、缓存/失败回退、实况、预警、7 日/24 小时预报、AQI、带伞建议和生活指数；首页显示开关与位置/温度/天气/AQI/短时降雨卡已接入；线上合肥接口及 API 24 构建通过；HarmonyOS 定位待补 |
